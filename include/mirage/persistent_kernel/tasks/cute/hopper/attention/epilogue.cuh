...
// https://github.com/flashinfer-ai/flashinfer/blob/bbb57add5affe44e5df87ecd2c97656108ef1330/include/flashinfer/attention/hopper/epilogue.cuh
...
template <int NUM_COPY_THREADS,
          ...
          typename TileShapeO,
          typename SMemO>
__forceinline__ __device__ void write_tiled(DTypeO *O,
                                            TiledCopyO const &tiled_copy_O,
                                            LayoutO const &layout_O,
                                            TileShapeO const &tile_shape_O,
                                            SMemO const &sO,
                                            int thread_idx,
                                            int qo_tile_idx,
                                            int qo_head_idx,
                                            ...
                                            int64_t qo_len) {
  Tensor mO =
      make_tensor(make_gmem_ptr(O + qo_indptr * stride<0>(layout_O)), layout_O);
  Tensor gO = get_local_tile_tensor(mO, tile_shape_O, qo_head_idx, 0, qo_len)(
      _, _, qo_tile_idx); // (O, D)
  Tensor cO =
      cute::make_identity_tensor(gO.shape()); // (O, D) -> (o_idx, d_idx)
  ...
  int const qo_tile_size = get<0>(tile_shape_O);
  int valid_qo_tile_size =
      std::min<int>(qo_len - qo_tile_idx * qo_tile_size, qo_tile_size);
...
template <int NUM_COPY_THREADS,
          ...
          typename TileShapeO,
          typename SMemO>
__forceinline__ __device__ void write_O(ElemO *O,
                                        TiledCopyO const &tiled_copy_O,
                                        LayoutO const &layout_O,
                                        TileShapeO const &tile_shape_O,
                                        SMemO const &sO,
                                        ...
                                        int qo_len,
                                        int write_warp_idx) {
  write_tiled<NUM_COPY_THREADS>(O,
                                tiled_copy_O,
                                layout_O,
                                tile_shape_O,
                                sO,
...
struct CollectiveEpilogue {
  ...
  using ThreadLayoutrO =
      Layout<Shape<_8, Int<CTA_Q / 16>, _4, _1>, Stride<_4, _32, _1, _0>>;
  using ValueLayoutrO =
      Layout<Shape<_1, _2, Shape<_2, _2>, Int<HEAD_DIM_VO / 16>>,
             Stride<_0, _2, Stride<_4, _1>, _8>>;
  using TiledCopyrO =
      decltype(make_tiled_copy(Copy_Atom<UniversalCopy<uint16_t>, DTypeO>{},
                               ThreadLayoutrO{},
                               ValueLayoutrO{}));
  using TiledCopyShaperO =
      Shape<_8, Int<CTA_Q / 8>, _16, Int<HEAD_DIM_VO / 16>>;
  using SmemLayoutrO =
  ...
  CUTLASS_DEVICE void store(Params const &epilogue_params,
    ...
    Tensor gLSE = get_lse_local_tile_tensor(
        mLSE, Shape<Int<CTA_Q>>{}, qo_head_idx, qo_indptr, qo_len)(_,
                                                                   qo_tile_idx);
    ...
    // tOcO has shape ((2, 2, V), MMA_M, MMA_K), we only take only the row
    // indices.
...
#pragma unroll
        for (int mi = 0; mi < size(lse); ++mi) {
          int const row = get<0>(tOcO_row(mi));
    ...
    write_O<NUM_COPY_THREADS>(epilogue_params.O_ptr,
                              gmem_tiled_copy_O,
  ...
  CUTLASS_DEVICE void store_zero(Params const &epilogue_params,
                                 SharedStorage &shared_storage,
                                 int thread_idx,
                                 BlockCoord const &block_coord) {
    auto [qo_tile_idx,
          ...
          kv_len,
          batch_idx] = block_coord;
    Tensor mO = make_tensor(make_gmem_ptr(epilogue_params.O_ptr),
                            epilogue_params.layout_O);
    Tensor gO = get_local_tile_tensor(
        mO, select<0, 1>(TileShape_PDV{}), qo_head_idx, qo_indptr, qo_len)(
        _, _, qo_tile_idx); // (O, D)
    Tensor cO =
        cute::make_identity_tensor(gO.shape()); // (O, D) -> (o_idx, d_idx)
    Tensor mLSE = make_tensor(make_gmem_ptr(epilogue_params.lse_ptr),
    ...
    int const qo_tile_size = get<0>(TileShape_PDV{});
    int valid_qo_tile_size =
        std::min<int>(qo_len - qo_tile_idx * qo_tile_size, qo_tile_size);
    if (valid_qo_tile_size == qo_tile_size) {
