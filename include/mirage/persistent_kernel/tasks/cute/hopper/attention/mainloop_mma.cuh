...
// https://github.com/flashinfer-ai/flashinfer/blob/bbb57add5affe44e5df87ecd2c97656108ef1330/include/flashinfer/attention/hopper/mainloop_mma.cuh
...
template <typename Ktraits,
          ...
          bool CAUSAL,
          bool MULTIITEMSCORING,
          typename WarpScheduler,
...
CUTLASS_DEVICE void mma_f16(Params const &mainloop_params,
                            ...
                            const int32_t qo_head_idx,
                            const int32_t kv_head_idx,
                            const uint32_t prefix_len,
                            uint16_t *token_pos_in_items,
                            int const num_kv_tiles_outside_items_window = 0,
                            int const num_kv_tiles_prefix = 0) {
  ...
  gemm</*init=*/true, /*wg_wait=*/-1>(
      tiled_mma_qk, tSrQ, tSrK(_, _, _, smem_pipe_read_k.index()), tSrS);
  WarpScheduler::barrier_arrive();

  if (work_idx != 0) {
    int lane_predicate = cute::elect_one_sync();
    if (cutlass::canonical_warp_idx_sync() == Ktraits::NUM_WARPS - 1 &&
        lane_predicate) {
#pragma unroll
  ...
  auto mask_multi_item_scoring =
      [&](decltype(tSrS) &tSrS, int i, int qo_idx, int kv_idx) {
        const uint32_t idx_in_original_seq = qo_idx + kv_len - qo_len;
        const bool out_of_boundary =
            kv_idx > idx_in_original_seq ||
            (kv_idx >= std::min(kv_len, col_limit_right(qo_idx)));
        const bool is_prefix = idx_in_original_seq < prefix_len;
        uint16_t token_pos_in_items_regs = 0;
        // Only access idx_in_original_seq >= prefix_len && idx_in_original_seq
        // < kv_len to avoid out-of-bounds memory access
        if (idx_in_original_seq >= prefix_len & idx_in_original_seq < kv_len) {
          token_pos_in_items_regs =
        ...
        if (out_of_boundary || is_prefix) {
          ...
          tSrS(i) = (kv_idx < prefix_len |
                     (idx_in_original_seq < kv_idx + token_pos_in_items_regs))
                        ? tSrS(i)
                        : (AttentionUpdater::fill_value);
        }
      };
  auto mask_multi_item_scoring_assume_in_bound =
      [&](decltype(tSrS) &tSrS, int i, int qo_idx, int kv_idx) {
        const uint32_t idx_in_original_seq = qo_idx + kv_len - qo_len;
        const bool is_prefix = idx_in_original_seq < prefix_len;
        if (is_prefix) {
          ...
          uint16_t token_pos_in_items_regs = 0;
          // Only access idx_in_original_seq >= prefix_len &&
          // idx_in_original_seq < kv_len to avoid out-of-bounds memory access
          if (idx_in_original_seq >= prefix_len &
              idx_in_original_seq < kv_len) {
            token_pos_in_items_regs =
                __ldca(token_pos_in_items + idx_in_original_seq - prefix_len);
          }
...
#pragma unroll
    for (int i = 0; i < size(tSrS); ++i) {
      ...
      tSrS(i) = variant.LogitsTransform(mainloop_params,
                                        tSrS(i),
                                        /*batch_idx=*/0,
                                        qo_idx,
                                        kv_idx,
  ...
  Tensor tOrP = make_tensor(
      convert_type<DTypeKV>(tSrS).data(),
  ...
  // masking loops
  // ziangl@nvidia.com: for multi item scoring, we use this loop only to mask
  // along the diagonal
#pragma unroll
  for (int masking_step = 0;
       masking_step < n_masking_steps && kv_tile_idx > swa_begin_kv_tile_idx;
...
#pragma unroll
    for (int i = 0; i < size(tSrS); ++i) {
      int qo_idx = get<0>(tScS(i)) + q_tile_idx * CTA_Q;
      int kv_idx =
          get<1>(tScS(i)) + kv_tile_idx_decrement(kv_tile_idx) * CTA_KV;
      tSrS(i) = variant.LogitsTransform(mainloop_params,
                                        tSrS(i),
                                        /*batch_idx=*/0,
                                        qo_idx,
    ...
    ++smem_pipe_read_v;
    cute::copy(
        make_tensor(convert_type<DTypeKV>(tSrS).data(),
                    convert_layout_acc_Aregs<typename Ktraits::TiledMmaPV>(
                        tSrS.layout())),
        tOrP);
  }

#pragma unroll 1
  for (; kv_tile_idx > swa_end_kv_tile_idx + 1;
...
#pragma unroll
    ...
    if constexpr (MULTIITEMSCORING) {
      // auto nums_tiles_outside_causal_diagonal = kv_tile_idx_count -
      // cute::ceil_div(CTA_Q, CTA_KV);
...
#pragma unroll
  ...
  cutlass::arch::NamedBarrier::arrive(
      NUM_MMA_THREADS + Ktraits::NUM_PRODUCER_THREADS,
