...
// https://github.com/flashinfer-ai/flashinfer/blob/bbb57add5affe44e5df87ecd2c97656108ef1330/include/flashinfer/attention/hopper/mainloop.cuh
...
struct CollectiveMainloop {
  ...
  static constexpr uint32_t TmaTransactionBytesV = static_cast<uint32_t>(
      size(take<0, 2>(SmemLayoutV{})) * cutlass::sizeof_bits_v<DTypeKV> / 8);

  // Whether use scheduler barrier or hardware warp scheduler, using heuristic
  // based on data type and head dim
  static constexpr bool UseSchedulerBarrier =
      cutlass::sizeof_bits_v<DTypeQ> == 8 ? HEAD_DIM_VO >= 128
                                          : HEAD_DIM_VO <= 128;
  ...
  static Params to_underlying_arguments(Arguments const &args) {
    ...
    TMA_K tma_load_K = make_tma_copy(GmemTiledCopyKV{},
                                     mK,
                                     SmemLayoutK{}(_, _, _0{}),
                                     select<1, 2>(TileShape_QKD{}),
                                     _1{}); // no mcast
    Tensor mV = make_tensor(make_gmem_ptr(args.V_ptr), args.layout_V);
    TMA_V tma_load_V = make_tma_copy(GmemTiledCopyKV{},
                                     mV,
                                     SmemLayoutV{}(_, _, _0{}),
                                     select<2, 1>(TileShape_PDV{}),
                                     _1{}); // no mcast
  ...
  CUTLASS_DEVICE
  int get_num_kv_tiles(Params const &mainloop_params,
                       int q_tile_idx,
                       int const qo_len,
                       int const kv_len) {
    static constexpr int CTA_Q = get<0>(TileShape_QKD{});
  ...
  CUTLASS_DEVICE void load(Params const &mainloop_params,
    ...
    Tensor gQ = get_local_tile_tensor(
        mQ, select<0, 2>(TileShape_QKD{}), qo_head_idx, qo_indptr, qo_len)(
        _, _, q_tile_idx); // (Q, D)
    Tensor gK = get_local_tile_tensor(mK,
                                      select<1, 2>(TileShape_QKD{}),
                                      kv_head_idx,
                                      kv_indptr,
                                      kv_len); // (K, D, _)
    Tensor gV = get_local_tile_tensor(mV,
                                      select<2, 1>(TileShape_PDV{}),
                                      kv_head_idx,
                                      kv_indptr,
                                      kv_len); // (K, D, _)
    ...
    auto [tKgK, tKsK] =
        tma_partition(mainloop_params.tma_load_K,
                      _0{},
                      Layout<_1>{},
                      group_modes<0, 2>(sK),
                      group_modes<0, 2>(gK)); // (TMA, k), (TMA, PIPE)
    auto [tVgV, tVsV] =
        tma_partition(mainloop_params.tma_load_V,
                      _0{},
                      Layout<_1>{},
                      group_modes<0, 2>(sV),
    ...
    int num_kv_tiles =
        get_num_kv_tiles(mainloop_params, q_tile_idx, qo_len, kv_len);
